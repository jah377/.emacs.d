...
function definitions. Alternatively, =sphinx-doc= makes it easier to insert [[https://sphinx-rtd-tutorial.readthedocs.io/en/latest/docstrings.html][Sphinx]]
style doc-strings. *N.B.* Prefer numpy-style doc-strings but
...
reformatting (~python-docstring-fill~) and highlighting Python
doc-strings. Text reformatted to =epytext= and =sphinx= formats, which conflicts
with =numpydoc=.
