...
inherits environment variables from your shell, particularly on systems like
macOS where Emacs launched from a GUI often does not inherit the user's shell
environment.

Here, we add our =tools= =venv= containing python applications. It is necessary to
add this path to ~exec-path~ as the applications will serve as the back-end for
various Emacs packages that provide, for instance, code formatting or LSP
functionality.
