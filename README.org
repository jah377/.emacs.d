...
My goal using =org-agenda= is to track the progress of ongoing personal- and
work-related =projects= and =tasks=. Here is what I mean by that:

+ Task :: "atomic" TODO item
  - Example:
    * Review a merge request
    * Add X package to my Emacs config

+ Project :: large TODO requiring the completion of several =tasks=
  - Example: Create an ML model
    * Task #1: Perform EDA
    * Task #2: Clean Data (guided by EDA)
    * Task #3: Train model
    * Task #4: Deploy API

For the sake of simplicity, TODO items are organized in a single =agenda.org=
file; "setting" context flags (discussed later) are used to quickly separate
tasks belonging to each file. Regardless of the file, top-level headers include
one-off =tasks= or =projects= containing multiple second-level =task=
headers. Additional context is provided via:

+ TODO state :: Action state of the item
  - Example :: ~TODO~, ~ONGOING~, ~HOLD/WAIT~, ~COMPLETE~, ~REVIEW~, ~MERGED~, ~DROPPED~

+ Tags :: Provide "setting"-, "action"-, or "subject"-specific context
  - Setting context :: Item related to work or personal
  ...
  - Action context :: Item requires an action by me
    - Prefix :: "!"
    - Example :: ~!review~, ~!respond~, ~!email~
  - Subject context :: General nature of item
    - Prefix :: None
    - Example :: ~bug~, ~note~, ~emacs~, ~devtools~, ~project~

+ Org-Header Property :: Any other details for the item
  - Example :: ~:git_issue:~, ~:branch:~, ~:repo:~
...
=org-tag-alist= variable defines list of TAG keywords. Calling ~M-x org-set-tags-command~ (~C-c
C-q~) sets the tags for the current visible entry.
