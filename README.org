...
This is my literate Emacs configuration file; there are many like it, but this
one is mine. It serves as both documentation and the source for configuring
Emacs. All necessary files to configure Emacs are generated from this file by
calling ~org-babel-tangle~ (~C-c C-v t~), and stored in the =/modules=
sub-directory. As such, changes should be made in this document, not in the
individual =.el= files.
...
displayed. Setting to ~'split-window-right~ ensures that the widow always opens
to the right of the current buffer. This is advantageous when we need to look
at previous =src-blocks= in the existing narrative.
...
*** Agenda TODO/Tag Settings

*N.B.* To increase flexibility, tags and todos are defined at the file-level and
not by configuring =org-tag-alist-for-agenda= or
...
files (see [[*Syntax Highlighting][Syntax Highlighting]]). To check a comment, we would need to remove
=org-block= in ~jinx-exclude-faces~ at the expense of checking ALL of the code.
